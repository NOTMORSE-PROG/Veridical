...
| Manuscript data privacy (PH Data Privacy Act) | Legal/ethical exposure | TLS + encrypted storage; no third-party sharing beyond citation metadata (only citation strings leave the system, never manuscript text except to Gemini — disclose this in the consent/ethics section). **The F7 originality corpus is shared, by design (owner decision, 2026-08-16, BUG-050): every ingested manuscript is inserted into one library and any manuscript may match any other, across instructor accounts. That is the feature — coverage grows with every insert.** What a match returns is bounded to a non-identifying reference (a record number), program, and processing date, plus a limited excerpt — **never the matched manuscript's real title, student names, team name, or file name, and never the full document** (owner ruling, 2026-08-11; identity withheld corrected 2026-09-04, BUG-147 — the Library screen previously returned every matched manuscript's full author names, capstone title, and team name to every instructor account with no consent step, which this table's own prior wording, "the matched manuscript's identity and a limited excerpt", had described as the intended behavior). **This corrects the earlier "instructor-only access" wording too, which described the schema's per-instructor scoping and not the corpus's actual behavior.** |
