...
# --- V-054: paired comparison of two grading configurations ------------------
...
def test_mcnemar_ignores_concordant_items_entirely():
    ...
    many = mcnemar_exact(
        PairedComparison(both_correct=500, only_a_correct=6, only_b_correct=0, both_wrong=99)
    )
...
def test_no_discordant_items_reports_indistinguishable_not_equivalent():
    ...
    assert "cannot distinguish" in result.note
...
def test_underpowered_run_is_flagged_rather_than_read_as_no_difference():
    ...
    assert "directional, not conclusive" in result.note


def test_required_discordant_pairs_is_the_floor_for_a_meaningful_ab_run():
